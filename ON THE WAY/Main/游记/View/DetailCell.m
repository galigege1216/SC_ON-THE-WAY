...
- (void)setActivity:(ActivityModel *)activity {
    ...
    NSDictionary *dic = _activity.contents[0];
    ...
    //滑动视图
    self.scrolView.contentSize = CGSizeMake(152 * (_activity.contents.count - 1)-2, kScrollViewHeight);
    self.scrolView.frame = CGRectMake(0, 200, kScreenWidth, kScrollViewHeight);
    for (int i = 0; i<activity.contents.count - 1; i++) {
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(i*152, 0, 150, kScrollViewHeight)];
        NSDictionary *contentDic = _activity.contents[i+1];
    ...
    //标题标签
    NSString *topic = _activity.topic;
...
- (NSUInteger)numberOfPhotosInPhotoBrowser:(WXPhotoBrowser *)photoBrowser{
    return _activity.contents.count;
}
...
- (WXPhoto *)photoBrowser:(WXPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index {
    ...
    NSDictionary *dic = _activity.contents[index];
    NSString *str = dic[@"photo_url"];
